...
if [[ $RUN_TYPE = LAN ]]; then
	ssh -i ~/.ssh/falcon-lan1.pem $USERNAME@$IP1 "pkill Falcon.out; echo clean completed; cd falcon-public; make all -j$(nproc); echo maked; chmod +x Falcon.out; ./Falcon.out 0 files/IP_$RUN_TYPE files/keyC files/keyAC files/keyBC $NETWORK $DATASET $SECURITY 1>./time.txt" & 
